    ...
    public final MapleMap getMap(final int mapid, final boolean respawns, final boolean npcs, final boolean reactors) {
        ...
        if (map == null) {
            ...
            try {
                ...
                // Rune
                final List<MapleMonster> monsters = map.getAllMonster();
                if (!map.isTown() && map.getAllMonster().size() > 0 && map.getAllMonster().get(0).getStats().getLevel() >= 30) {
                    MapleMonster mob = (MapleMonster) monsters.get(Randomizer.rand(0, monsters.size() - 1));
                    MapleRune rune = new MapleRune(Randomizer.rand(0, 3), mob.getPosition().x, mob.getPosition().y, map);//Randomizer.rand(map.getFootholds().getX1(), map.getFootholds().getX2()), Randomizer.rand(map.getFootholds().getY2(), map.getFootholds().getY1()), map);
                    map.spawnRune(rune);
                }
