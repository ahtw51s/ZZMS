...
public class MapleInventoryManipulator {
    ...
    public static void addRing(MapleCharacter chr, int itemId, int ringId, int sn, String partner) {
        ...
        chr.getClient().getSession().write(CSPacket.sendBoughtRings(ItemConstants.類型.戀人戒指(itemId), ring, sn, chr.getClient().getAccID(), partner));
    }
    ...
    public static Item checkEnhanced(final Item before, final MapleCharacter chr) {
        if (before instanceof Equip) {
            ...
            if (eq.getState(false) == 0 && (eq.getUpgradeSlots() >= 1 || eq.getLevel() >= 1) && ItemConstants.卷軸.canScroll(eq.getItemId()) && Randomizer.nextInt(100) >= 80) { //20%概率潛能掉寶
    ...
    public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
        ...
        if (!type.equals(MapleInventoryType.EQUIP) && initialTarget != null
                ...
                && !ItemConstants.類型.可充值道具(source.getItemId())
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (ItemConstants.類型.雙刀(source.getItemId()) || source.getItemId() / 10000 == 135) {
        ...
        switch (dst) {
            case -6: { // Bottom
                ...
                if (top != null && ItemConstants.類型.套服(top.getItemId())) {
            ...
            case -5: {
                ...
                if (bottom != null && ItemConstants.類型.套服(source.getItemId())) {
            ...
            case -10: { // Shield
                ...
                if (ItemConstants.類型.雙刀(source.getItemId())) {
                    if ((chr.getJob() != 900 && !MapleJob.is影武者(chr.getJob())) || weapon == null || !ItemConstants.類型.短劍(weapon.getItemId())) {
                        c.getSession().write(InventoryPacket.getInventoryFull());
                        c.getSession().write(InventoryPacket.getShowInventoryFull());
                        return;
                    }
        ...
        if (source.getState(false) >= 17) {
            ...
            int[] potentials = {source.getPotential(1, false), source.getPotential(2, false), source.getPotential(3, false), source.getPotential(1, true), source.getPotential(2, true), source.getPotential(3, true)};
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced) {
        ...
        final Item source = c.getPlayer().getInventory(type).getItem(src);
        if (quantity < 0 || source == null || src == -55 || (!npcInduced && ItemConstants.類型.寵物(source.getItemId())) || (quantity == 0 && !ItemConstants.類型.可充值道具(source.getItemId())) || c.getPlayer().inPVP()) {
            c.getSession().write(CWvsContext.enableActions());
            return false;
        }
