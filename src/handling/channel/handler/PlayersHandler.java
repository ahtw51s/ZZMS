    ...
    public static final void TouchRune(final LittleEndianAccessor slea, final MapleCharacter chr) {
        ...
        if (rune != null && rune.getRuneType() == type) {
            if (chr.getKeyValue("LastTouchedRune") != null && chr.getRuneTimeStamp() > System.currentTimeMillis()) {
                chr.getClient().getSession().write(CField.RunePacket.RuneAction(2, (int) (chr.getRuneTimeStamp() - System.currentTimeMillis())));
                chr.getClient().getSession().write(CWvsContext.enableActions());
                return;
            }
            chr.setTouchedRune(type);
            chr.getClient().getSession().write(CField.RunePacket.RuneAction(5, 0));
        } else {
        ...
        if (result == 1) {
            switch (chr.getTouchedRune()) {
                case 0: //疾速之輪
                    ...
                    effect.applyTo(chr);
                    chr.getClient().getSession().write(CWvsContext.getMapleTipMsg("疾速之輪:提升所有速度獲得追加經驗值."));
                    break;
                case 1: //再生之輪
                    effect = SkillFactory.getSkill(80001428).getEffect(1);
                    effect.applyTo(chr);
                    chr.getClient().getSession().write(CWvsContext.getMapleTipMsg("再生之輪: 提升再生力獲得追加經驗值."));
                    break;
                case 2: //崩壞之輪
                    effect = SkillFactory.getSkill(80001430).getEffect(1);
                    effect.applyTo(chr);
                    chr.getClient().getSession().write(CWvsContext.getMapleTipMsg("崩壞之輪: 強化攻擊力獲得追加經驗值.."));
                    break;
                case 3: //破滅之輪
                    effect = SkillFactory.getSkill(80001432).getEffect(1);
                    effect.applyTo(chr);
                    chr.getClient().getSession().write(CWvsContext.getMapleTipMsg("破滅之輪: 強化攻擊力獲得追加經驗值."));
                    break;
            }
